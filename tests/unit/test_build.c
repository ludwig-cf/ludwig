...
int test_build_update_links_with_state(pe_t * pe, cs_t * cs);
...
static int test_build_update_map_sph_with_state(pe_t * pe, cs_t * cs, double a0,
                                     const double r0[3]);
...
int test_build_suite(void) {
  ...
  test_build_update_links_with_state(pe, cs);
...
/*****************************************************************************
 *
 *  test_build_update_map_with_state
 *
 *  At the moment build_update_map() updates a number of things:
 *    1. map status
 *    2. map data wetting values (not always relevant)
...
int test_build_update_map_with_state(pe_t * pe, cs_t * cs) {
...

  {
    ...
    ifail = test_build_update_map_sph_with_state(pe, cs, a0, r0);
    assert(ifail == 0);
  }
...

  {
    ...
    ifail = test_build_update_map_ell_with_state(pe, cs, abc, r0, q4);
    assert(ifail == 0);
  }
...
/*****************************************************************************
 *
 *  test_build_update_links_with_state
 *
 *  Again, build_update_links() performs a number of different operations.
 *  Here we are concerned with just the generation of a consistent
 *  number of links (really not much more than a smoke test).
...
int test_build_update_links_with_state(pe_t * pe, cs_t * cs) {
...

  {
    ...
    ifail = test_build_update_links_sph_with_state(pe, cs, a0, r0, 15);
    assert(ifail == 0);
    ifail = test_build_update_links_sph_with_state(pe, cs, a0, r0, 19);
    assert(ifail == 0);
    ifail = test_build_update_links_sph_with_state(pe, cs, a0, r0, 27);
    assert(ifail == 0);
  }
...
/*****************************************************************************
 *
 *  test_build_update_map_sph_with_state
 *
 *  Utility to test map against discrete volume for spheres.
 *
 *****************************************************************************/
...
static int test_build_update_map_sph_with_state(pe_t * pe, cs_t * cs, double a0,
...

  {
    ...
    colloids_info_add_local_with_state(cinfo, &s, &pc);
    if (pc) pc->s = s;
  }
...
/*****************************************************************************
 *
 *  test_buld_update_map_ell_with_state
 *
 *  Utility to test map against discrete volume for ellipsoids
 *
 *****************************************************************************/
...
/*****************************************************************************
 *
 *  test_build_update_links_sph_with_state
 *
 *  A utiity to check the number of links generated for the given model
 *  (nvel) is consistent. This is limited to COLLOID_SHAPE_SPHERE
 *  occupying a single site.

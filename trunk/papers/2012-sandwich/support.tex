...
\end{center}
\caption{Particle distribution during and after switched electric field direction: 
The above images show the ordering with the electric field along the x-direction 
after a previous run that had the field along z. For clarity disclination lines have
been removed. The left panels show the configuration and the structure factor 
while the field is on, whereas the righ panels show the same quantities after 
switch-off. The structure factor data shows wave vectors $k_y, k_z \in [-3\pi/8 , 3\pi/8]$.}
\end{figure}

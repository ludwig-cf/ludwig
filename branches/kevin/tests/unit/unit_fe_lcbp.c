...
int do_test_fe_lcbp_param(control_t * ctrl);
...
/*****************************************************************************
 *
 *  do_test_fe_lcbp_param
 *
 *****************************************************************************/

int do_test_fe_lcbp_param(control_t * ctrl) {

  fe_lcbp_param_t param0 = {0.014384711,
                            3.1764706,
                            0.01, 0.01,
                            0.27768018,
                            0.7,
  ...
  control_verb(ctrl, "Liquid crystal BP parameter test\n");

  pe_create_parent(MPI_COMM_WORLD, &pe);
  coords_create(pe, &cs);
  coords_commit(cs);

  field_create(cs, 1, "q", &q);
  field_grad_create(q, 0, &dq);

  fe_lcbp_create(q, dq, &lcbp);

  try {

    fe_lcbp_param_set(lcbp, param0);
    fe_lcbp_param(lcbp, &param1);

    control_verb(ctrl, "Parameter A0:       %22.15e %22.15e\n",
		 param0.a0, param1.a0);
    control_macro_test_dbl_eq(ctrl, param0.a0, param1.a0, DBL_EPSILON);
    control_verb(ctrl, "Parameter gamma:    %22.15e %22.15e\n",
		 param0.gamma, param1.gamma);
    control_macro_test_dbl_eq(ctrl, param0.gamma, param1.gamma, DBL_EPSILON);

    /* We will assume the other params in struct are ok. */

    /* Dependent parameters */
    /* Note q0 must be inside sqrt() to pass at DBL_EPSILON */

    chi1 = sqrt(108.0*param0.kappa0*param0.q0*param0.q0/(param0.a0*param0.gamma));
    fe_lcbp_chirality(lcbp, &chi0);

    control_verb(ctrl, "Chirality:          %22.15e %22.15e\n", chi0, chi1);
    control_macro_test_dbl_eq(ctrl, chi0, chi1, DBL_EPSILON);

    tau1 = 27.0*(1.0 - param0.gamma/3.0)/param0.gamma;
    fe_lcbp_reduced_temperature(lcbp, &tau0);

    control_verb(ctrl, "Red. temp. tau:     %22.15e %22.15e\n", tau0, tau1);
    control_macro_test_dbl_eq(ctrl, tau0, tau1, DBL_EPSILON);

    /* a = (2/3)*(1/4 + 3/4*sqrt(1 - 8/3\gamma)) */

    amp1 = (2.0/3.0)*(0.25 + 0.75*sqrt(1.0 - 8.0/(3.0*param0.gamma)));

    fe_lcbp_amplitude_compute(lcbp, &amp0);
    control_verb(ctrl, "Min. nematic amp0:  %22.15e %22.15e\n", amp0, amp1);
    control_macro_test_dbl_eq(ctrl, amp0, amp1, DBL_EPSILON);

    /* Dimensionless field */

    edm1 = sqrt((27.0*param0.epsilon/(32.0*pi_*param0.a0*param0.gamma))
		*util_dot_product(param0.electric, param0.electric));
    fe_lcbp_dimensionless_field_strength(lcbp, &edm0);

    control_verb(ctrl, "Dimensionless fld:  %22.15e %22.15e\n", edm0, edm1);
    control_macro_test_dbl_eq(ctrl, edm0, edm1, DBL_EPSILON);

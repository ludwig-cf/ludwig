...
TARGET void lb_collision_binary_site( double* __restrict__ t_f, 
   ...
   * where each holds a vector "chunk" of lattice sites to allow SIMD
   * operations.
   * e.g. "DECLARE_SIMD_VECTOR1D(type, var, size, iv) is equivalent of
   * "type var[size]", where the extra index iv will automatically index
   * into the short SIMD array when used in conjuction with TARGET_ILP.  
   */
  ...
  /* Initialise fluctuations shat[i][j] and ghat[mode] to zero */
  ...
  /* Compute all the modes from the distributions */

  for (m = 0; m < tc_nmodes_; m++) {
    TARGET_ILP(iv) SIMD_1D_ELMNT(mode, m, iv) = 0.0;
    for (p = 0; p < NVEL; p++) {
      TARGET_ILP(iv) {
  	index = LB_ADDR(tc_nSites, NDIST, NVEL, baseIndex + iv, 0, p);
	SIMD_1D_ELMNT(mode, m, iv) += t_f[index]*tc_ma_[m][p];
      }
    } 
  }
  ...
  TARGET_ILP(iv) SIMD_SC_ELMNT(rrho,iv) = 1.0/SIMD_SC_ELMNT(rho,iv);
  ...
  for (i = 0; i < 3; i++) {
    ...
    /* ... so that seq[i][j] = rho*u[i]*u[j] + sth[i][j] */
  ...
  /* Relax each mode */
  /* Here, the trace is relaxed tr_s = tr_s - rtau_bulk*(tr_s - tr_seq) */

  TARGET_ILP(iv) {
    SIMD_SC_ELMNT(tr_s,iv) = SIMD_SC_ELMNT(tr_s,iv)
      - tc_rtau_bulk*(SIMD_SC_ELMNT(tr_s,iv) - SIMD_SC_ELMNT(tr_seq,iv));
  }

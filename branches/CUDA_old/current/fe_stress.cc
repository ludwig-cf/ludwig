...
// average shear stress: y - flow direction, z - gradient direction

	avestress+=Stressyz[i][j][k];
...
//   if (REDSHIFT==1 && n>20000){
   ...
   if (REDSHIFT==1){
      ...
      if(fabs(rr)<1e-9 || isnan(rr)==1){rr=rr_prev;}
   }
...
/* free energy and redshift output */
   ...
   if (n % (FePrintInt) == 0){
	 ...
	 file << n << "\t" << freeenergy/Lx/Ly/Lz << " " << freeenergytwist/Lx/Ly/Lz << "\t" << avestress/Lx/Ly/Lz << "\t" << rr <<"\t" << phivr << "\t" << noise_strength << endl;
	 file.close();
       }
   }

...
static int nernst_planck_fluxes_d3q18(psi_t * psi, double ** flx, map_t *map);
static int nernst_planck_update_d3q18(psi_t * psi, double ** flx, map_t * map, double dt);
...
int nernst_planck_driver_d3q18(psi_t * psi, hydro_t * hydro, map_t * map, double dt) {
  ...
  nernst_planck_fluxes_d3q18(psi, flx, map);
  
  /* Apply no-flux BC */
  if (map) advective_bcs_no_flux_d3q18(nk, flx, map);

  /* Update charges */
  nernst_planck_update_d3q18(psi, flx, map, dt);

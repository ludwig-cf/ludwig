...
static void gradient_bcs6x5(double kappa0, double kappa1, const int dn[3],
			    double dq[NQAB][3], double bc[6][NQAB][3]);
...
static void gradient_bcs6x5_block(double kappa0, double kappa1,
				  const int dn[3],
				  double dq[NQAB], double bc[6][NQAB],
				  int id);
...
static void gradient_general6(const double * field, double * grad,
  ...
  for (ic = 1 - nextra; ic <= nlocal[X] + nextra; ic++) {
	...
	map_status(map_, coords_index(ic+1, jc, kc), status);
	map_status(map_, coords_index(ic-1, jc, kc), status + 1);
	map_status(map_, coords_index(ic, jc+1, kc), status + 2);
	map_status(map_, coords_index(ic, jc-1, kc), status + 3);
	map_status(map_, coords_index(ic, jc, kc+1), status + 4);
	map_status(map_, coords_index(ic, jc, kc-1), status + 5);
	...
	for (n = 0; n < 6; n++) {
	  ...
	  if (status[n] == MAP_COLLOID) {
	    w1 = w1_coll;
	    w2 = w2_coll;
	  }

	  if (status[n] == MAP_BOUNDARY) {
	    w1 = w1_wall;
	    w2 = w2_wall;
	  }
	...
	for (niterate = 0; niterate < NMAX_ITERATION; niterate++) {
	  ...
	  del2diff = 0.0;
	  for (n1 = 0; n1 < NQAB; n1++) {
	    del2diff += fabs(del2[NQAB*index + n1] - del2m1[n1]);
	  }
...
static void gradient_general6x5(const double * field, double * grad,
  ...
  for (ic = 1 - nextra; ic <= nlocal[X] + nextra; ic++) {
	...
	map_status(map_, coords_index(ic+1,jc,kc), status + 0);
	map_status(map_, coords_index(ic-1,jc,kc), status + 1);
	map_status(map_, coords_index(ic,jc+1,kc), status + 2);
	map_status(map_, coords_index(ic,jc-1,kc), status + 3);
	map_status(map_, coords_index(ic,jc,kc+1), status + 4);
	...
	for (niterate = 0; niterate < NMAX_ITERATION; niterate++) {
	  ...
	  for (n = 0; n < 6; n++) {
	    ...
	    {
	      int ifail;
	      ifail = util_svd_solve(6, NQAB, a, b, x);
...
static int gradient_no_iteration(const double * field, double * grad,
  ...
  coords_nlocal(nlocal);
  util_matrix_create(6, NQAB, &a);
  util_matrix_create(2*6, 2*NQAB, &a2);
  util_matrix_create(3*6, 3*NQAB, &a3);
  ...
  for (ic = 1 - nextra; ic <= nlocal[X] + nextra; ic++) {
	...
	n = 0;
	map_status(map_, coords_index(ic+1,jc,kc), status + n);
	if (status[n] != MAP_FLUID) {
	  normal[n] = 0; /* +X */
	  n += 1;
	};
	map_status(map_, coords_index(ic-1,jc,kc), status + n);
	...
	map_status(map_, coords_index(ic,jc+1,kc), status + n);
	if (status[n] != MAP_FLUID) {
	  normal[n] = 2; /* +Y */
	  n += 1;
	}
	...
	map_status(map_, coords_index(ic,jc,kc+1), status + n);
	if (status[n] != MAP_FLUID) {
	  normal[n] = 4; /* +Z */
	  n += 1;
	}
	...
	if (nunknown == 0) {
	  ...
	  if (nunknown == 1) {
		...
		for (n1 = 0; n1 < 6; n1++) {
		  for (n2 = 0; n2 < NQAB; n2++) {
		    a2[6*ia + n1][NQAB*ib + n2] = f*bc1[n1][n2];
		  }
		}
	    ...
	    ifail = util_svd_solve(2*6, 2*NQAB, a2, b2, x2);
		...
		for (n1 = 0; n1 < 6; n1++) {
		  for (n2 = 0; n2 < NQAB; n2++) {
		    a3[6*ia + n1][NQAB*ib + n2] = f*bc1[n1][n2];
		  }
		}
	    ...
	    ifail = util_svd_solve(3*6, 3*NQAB, a3, b3, x3);
	    if (ifail != 0) fatal("SVD failed n = 3\n");

	    for (n = 0; n < nunknown; n++) {
	      for (n1 = 0; n1 < NQAB; n1++) {
		gradn[n1][normal[n]/2][normal[n] % 2] = x3[NQAB*n + n1];
	      }
	    }

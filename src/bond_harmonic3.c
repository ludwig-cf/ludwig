...
int bond_harmonic3_compute(colloids_info_t * cinfo, void * self) {
  ...
  for (; pc; pc = pc->nextlocal) {
    ...
    for (n = 0; n < pc->s.nbonds3; n++) {
      //pe_verbose(cinfo->pe, "index %d,  %d bonded %d %d %d %d %d %d \n", pc->s.index, pc->s.nbonds3, pc->bonded3[0]->s.index, pc->bonded3[1]->s.index, pc->bonded3[2]->s.index, pc->bonded3[3]->s.index, pc->bonded3[4]->s.index, pc->bonded3[5]->s.index);
      assert(pc->bonded3[n]);

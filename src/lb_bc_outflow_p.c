...
__host__ int lb_bc_outflow_p_init_internal(lb_bc_outflow_p_t * outflow);
__host__ int lb_bc_outflow_init_link(lb_bc_outflow_p_t * outflow,
				     link_init_enum_t flag, int id);

static int int_max(int a, int b) {return (a > b) ? a : b;}

static const lb_bc_open_vtable_t vt_ = {
  (lb_bc_open_free_ft)   lb_bc_outflow_p_free,
  (lb_bc_open_update_ft) lb_bc_outflow_p_update,
  (lb_bc_open_impose_ft) lb_bc_outflow_p_impose,
  (lb_bc_open_stats_ft)  lb_bc_outflow_p_stats
};

/*****************************************************************************
 *
 *  lb_bc_outflow_p_create
 *
 *****************************************************************************/

__host__ int lb_bc_outflow_p_create(pe_t * pe, cs_t * cs,
				       const lb_bc_outflow_opts_t * options,
				       lb_bc_outflow_p_t ** outflow) {
  lb_bc_outflow_p_t * bc = NULL;
  ...
  bc = (lb_bc_outflow_p_t *) calloc(1, sizeof(lb_bc_outflow_p_t));
  assert(bc);
  if (bc == NULL) pe_fatal(pe, "Failed to allocate lb_bc_outflow_p_t");
  ...
  bc->super.func = &vt_;
  bc->super.id   = LB_BC_OUTFLOW_P;

  if (!lb_bc_outflow_opts_valid(*options)) {
    /* Internal error if we reach this point. */
    pe_fatal(pe, "lb_bc_outflow_p_create: options invalid\n");
  }
  bc->options = *options;

  lb_bc_outflow_p_init_internal(bc);
...
/*****************************************************************************
 *
 *  lb_bc_outflow_p_free
 *
 *****************************************************************************/

__host__ int lb_bc_outflow_p_free(lb_bc_outflow_p_t * outflow) {
...
__host__ int lb_bc_outflow_p_update(lb_bc_outflow_p_t * outflow,
				      hydro_t * hydro) {
...
__host__ int lb_bc_outflow_p_halo_update(lb_bc_outflow_p_t * outflow,
					    hydro_t * hydro) {
  cs_t * cs = NULL;
  int noffset[3] = {0};
  int ntotal[3] = {0};
...
__host__ int lb_bc_outflow_p_impose(lb_bc_outflow_p_t * outflow,
				       hydro_t * hydro,
				       lb_t * lb) {
  assert(outflow);
  assert(hydro);
  ...
  if (pe_mpi_size(outflow->pe) > 1) {
    lb_bc_outflow_p_halo_update(outflow, hydro);
  }

  /* For each link at outflow */

  for (int n = 0; n < outflow->nlink; n++) {

    int i    = outflow->linki[n];        /* boundary region */
    int j    = outflow->linkj[n];        /* fluid region */
    int8_t p = outflow->linkp[n];        /* i -> j */

    double rho0 = outflow->options.rho0; /* Inflow value */
    double u[3] = {};                    /* Velocity at boundary u(i) */ 
...

    {
      ...
      hydro_rho(hydro, j, &rho);
      hydro_u(hydro, j, v);

      for (int ia = 0; ia < 3; ia++) {
	vdotc += v[ia]*lb->model.cv[pj][ia];
	for (int ib = 0; ib < 3; ib++) {
	  double d_ab = (ia == ib);
	  double s_ab = lb->model.cv[pj][ia]*lb->model.cv[pj][ib] - cs2*d_ab;

	  sdotq += s_ab*v[ia]*v[ib];
	}
      }

      /* Non equilibrium part of outflowing distribution */
      /* requires f_eq(rho(j), u(j)) */

      double fout = 0.0;
      double fout_eq = 0.0;
      lb_f(lb, j, NVEL - p, LB_RHO, &fout);
      fout_eq = rho*lb->model.wv[p]*(1.0 + rcs2*vdotc + 0.5*rcs2*rcs2*sdotq);
	...
	for (int ib = 0; ib < 3; ib++) {
      ...
      {
	/* Here's the equilibrium for new inflowing distribution
	 * is f(rho0, u) */
	fp = rho0*lb->model.wv[p]*(1.0 + rcs2*udotc + 0.5*rcs2*rcs2*sdotq);
	fp = fp + (fout - fout_eq);
	lb_f_set(lb, i, p, LB_RHO, fp);
	fnet += (fout - fp);
      }
    }
  }

  //printf("Net out: %14.7e\n", fnet);
...
/*****************************************************************************
 *
 *  lb_bc_outflow_p_stats
 *
 *  Placeholder. No operation at the moment.
 *
 *****************************************************************************/

__host__ int lb_bc_outflow_p_stats(lb_bc_outflow_p_t * outflow) {

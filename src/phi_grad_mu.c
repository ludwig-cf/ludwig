...
__global__ void phi_grad_mu_solid_kernel(kernel_ctxt_t * ktx, field_t * phi,
					 fe_t * fe, hydro_t * hydro,
					 map_t * map);
__global__ void phi_grad_mu_solid_with_colloid_force_kernel(kernel_ctxt_t * ktx, field_t * phi,
					 fe_t * fe, hydro_t * hydro,
					 map_t * map, double sumforce[3]);
...
__host__ int phi_grad_mu_solid(cs_t * cs, field_t * phi, fe_t * fe,
  ...
  if (ncolloid == 1) {
    tdpLaunchKernel(phi_grad_mu_solid_with_colloid_force_kernel, nblk, ntpb, 0, 0,
		  ctxt->target, phi->target, fe_target, hydro->target,
		  map->target, sumforce);
...
/*****************************************************************************
 *
 *  phi_grad_mu_solid_with_force_kernel
 *
 *  This computes and stores the force on the fluid via
 *    f_a = - phi \nabla_a mu
 *
...
__global__ void phi_grad_mu_solid_with_colloid_force_kernel(kernel_ctxt_t * ktx, field_t * field,
					 fe_t * fe, hydro_t * hydro,
					 map_t * map, double sumforce[3]) {
  int kiterations;
  int kindex;

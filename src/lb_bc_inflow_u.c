...
int lb_bc_inflow_u_init_internal(lb_bc_inflow_u_t * inflow);
int lb_bc_inflow_init_link(lb_bc_inflow_u_t * inflow,
			   link_init_enum_t flag, int id);

static int int_max(int a, int b) {return (a > b) ? a : b;}

static const lb_bc_open_vtable_t vt_ = {
  (lb_bc_open_free_ft)   lb_bc_inflow_u_free,
  (lb_bc_open_update_ft) lb_bc_inflow_u_update,
  (lb_bc_open_impose_ft) lb_bc_inflow_u_impose,
  (lb_bc_open_stats_ft)  lb_bc_inflow_u_stats
};

int lb_bc_inflow_u_halo_update(lb_bc_inflow_u_t * inflow, hydro_t * hydro);

/*****************************************************************************
 *
 *  lb_bc_inflow_u_create
 *
 *****************************************************************************/
...
int lb_bc_inflow_u_create(pe_t * pe, cs_t * cs,
  ...
  bc = (lb_bc_inflow_u_t *) calloc(1, sizeof(lb_bc_inflow_u_t));
  assert(bc);
  if (bc == NULL) pe_fatal(pe, "Failed to allocate lb_bc_inflow_u_t");
  ...
  bc->super.func = &vt_;
  bc->super.id   = LB_BC_INFLOW_U;

  if (!lb_bc_inflow_opts_valid(*options)) {
    /* Internal error if we reach this point. */
    pe_fatal(pe, "lb_bc_inflow_u_create: lb_openbc_options_t invalid\n");
  }
  bc->options = *options;

  lb_bc_inflow_u_init_internal(bc);
...
__host__ int lb_bc_inflow_init_link(lb_bc_inflow_u_t * inflow,
...
int lb_bc_inflow_u_update(lb_bc_inflow_u_t * inflow, hydro_t * hydro) {
  ...
  if (noffset[id] == 0) {
	...
	for (int kc = 1; kc <= nlocal[Z]; kc++) {
	  ...
	  {
	    double y  = 1.0*jc;
	    double ly = 1.0*cs->param->ntotal[Y];
	    double yc = 0.5 + 0.5*ly;
	    double ux = 1.0 - 4.0*(y - yc)*(y - yc)/(ly*ly);

	    double u0[3] = {inflow->options.u0[X]*ux, 0.0, 0.0};

	    hydro_u_set(hydro, index0, u0);
...
int lb_bc_inflow_u_halo_update(lb_bc_inflow_u_t * inflow,
					   hydro_t * hydro) {
  cs_t * cs = NULL;
  int noffset[3] = {0};
  int nlocal[3]  = {0};
...
int lb_bc_inflow_u_impose(lb_bc_inflow_u_t * inflow, hydro_t * hydro,
  ...
  if (pe_mpi_size(inflow->pe) > 1) {
    lb_bc_inflow_u_halo_update(inflow, hydro);
  }
  ...
  for (int n = 0; n < inflow->nlink; n++) {
    ...
    int i     = inflow->linki[n];
    int j     = inflow->linkj[n];
	...
	udotc += uj[ia]*lb->model.cv[pj][ia];
	for (int ib = 0; ib < 3; ib++) {
	  double d_ab = (ia == ib);
	  double s_ab = lb->model.cv[pj][ia]*lb->model.cv[pj][ib] - cs2*d_ab;

	  sdotq += s_ab*uj[ia]*uj[ib];
	}
      }
    
      /* Non equilibrium part of outcoming distribution */
      /* requires f_eq(rho(j), u(j)) */
    
      lb_f(lb, j, pj, LB_RHO, &fout);
      fout_eq = rho*lb->model.wv[pj]*(1.0 + rcs2*udotc + 0.5*rcs2*rcs2*sdotq);
    }

    /* Incoming equilibrium f_eq(rho(j), u0(i)) */
    {
      double udotc = 0.0;
      double sdotq = 0.0;
      double u[3]  = {};
	...
	fin = rho*lb->model.wv[p]*(1.0 + rcs2*udotc + 0.5*rcs2*rcs2*sdotq);
	fin = fin + (fout - fout_eq);
	lb_f_set(lb, i, p, LB_RHO, fin);

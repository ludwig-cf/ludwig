...
struct ludwig_s {
  ...
  fe_ternary_t * fe_ternary;   /* Ternary free energy*/
...
void ludwig_run(const char * inputfile) {
  ...
  pe_info(ludwig->pe, "Starting time step loop.\n");
   pe_info(ludwig->pe, "222222");
  subgrid_on(&is_subgrid);
  pe_info(ludwig->pe, "is_subgrid");
  ...
  while (physics_control_next_step(ludwig->phys)) {
    ...
    printf("TIMER_start(TIMER_STEPS);");
    step = physics_control_timestep(ludwig->phys);
    printf("physics_control_timestep(ludwig->phys));");
    if (ludwig->hydro) {
   ...
   printf("ludwig->hydro");
    ...
    printf("ludwig_colloids_update(ludwig);");
    /* Order parameter gradients */
      printf("TIMER_PHI_GRADIENTS");

...
int field_phi_init_spinodal_patches(field_t * phi, int seed, int patch,
                                    double volminus1,double volminus2) {
    ...
    for (ic = 1; ic <= nlocal[X]; ic += patch) {
        for (jc = 1; jc <= nlocal[Y]; jc += patch) {
            for (kc = 1; kc <= nlocal[Z]; kc += patch) {
                ...
                if (ran_uniform < volminus1) {phi1 = -1.0;}
                else if (ran_uniform >= volminus1 && ran_uniform < volminus2){ phi1=0.0; }
                else {phi1 = 1.0;}
...
/*****************************************************************************
 *
 *  field_phi_init_ternary_block_XY
 *
 *  Initialise one block of chosen thickness at central position on X
 *
 *****************************************************************************/
    ...
    for (ic = 1; ic <= nlocal[X]; ic++) {
        for (jc = 1; jc <= 0.5*nlocal[Y]; jc++) {
            for (kc = 1; kc <= nlocal[Z]; kc++) {
                ...
                if ((x-0.5*len[X])*(x-0.5*len[X])+(y-0.5*len[Y])*(y-0.5*len[Y]) <= 0.25*len[Y]*0.25*len[Y]) {
                    phi0 = 0.0;
                }
                else {
                    phi0 = 1.0;

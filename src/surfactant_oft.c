 ...
 *  fe_surfactant_oft.c
 *
 *  Implementation of the temperature-dependent surfactant free energy described by
...
static fe_vt_t fe_surf_oft_hvt = {
  (fe_free_ft)      fe_surf_oft_free,     /* Virtual destructor */
  (fe_target_ft)    fe_surf_oft_target,   /* Return target pointer */
  (fe_fed_ft)       fe_surf_oft_fed,      /* Free energy density */
  (fe_mu_ft)        fe_surf_oft_mu,       /* Chemical potential */
  (fe_mu_solv_ft)   NULL,
  (fe_str_ft)       fe_surf_oft_str,      /* Total stress */
  (fe_str_ft)       fe_surf_oft_str,      /* Symmetric stress */
  ...
  (fe_stress_v_ft)  fe_surf_oft_str_v,    /* Total stress (vectorised version) */
  (fe_stress_v_ft)  fe_surf_oft_str_v,    /* Symmetric part (vectorised) */
  (fe_stress_v_ft)  NULL              /* Antisymmetric part */
};


static __constant__ fe_surf_oft_param_t const_param;
...
__host__ int fe_surf_oft_create(pe_t * pe, cs_t * cs, field_t * phi,
  ...
  obj = (fe_surf_oft_t *) calloc(1, sizeof(fe_surf_oft_t));
  assert(obj);
  if (obj == NULL) pe_fatal(pe, "calloc(fe_surf_oft_t) failed\n");

  obj->param = (fe_surf_oft_param_t *) calloc(1, sizeof(fe_surf_oft_param_t));
  assert(obj->param);
  if (obj->param == NULL) pe_fatal(pe, "calloc(fe_surf_oft_param_t) failed\n");
  ...
  obj->super.func = &fe_surf_oft_hvt;
  obj->super.id = FE_SURFACTANT_OFT;
  ...
  if (ndevice == 0) {
    fe_surf_oft_param_set(obj, param);
    obj->target = obj;
  }
  else {
    fe_surf_oft_param_t * tmp;
    tdpMalloc((void **) &obj->target, sizeof(fe_surf_oft_t));
    tdpGetSymbolAddress((void **) &tmp, tdpSymbol(const_param));
    tdpMemcpy(&obj->target->param, tmp, sizeof(fe_surf_oft_param_t *),
...
__host__ int fe_surf_oft_info(fe_surf_oft_t * fe) {
  ...
  fe_surf_oft_sigma(fe, &sigma);
  fe_surf_oft_xi0(fe, &xi0);
  fe_surf_oft_langmuir_isotherm(fe, &psi_c);
...
__host__ int fe_surf_oft_param(fe_surf_oft_t * fe, fe_surf_oft_param_t * values) {
  assert(fe);
...
__host__ int fe_surf_oft_str(fe_surf_oft_t * fe, int index, double s[3][3]) {
  ...
  for (ia = 0; ia < 3; ia++) {
    for (ib = 0; ib < 3; ib++) {
      s[ia][ib] = p0*d[ia][ib]	+
	(kappaoft - fe->param->epsilon*psi - fe->param->beta*psi*psi)*
	grad[0][ia]*grad[0][ib];
    }
  }
...
int fe_surf_oft_str_v(fe_surf_oft_t * fe, int index, double s[3][3][NSIMDVL]) {
  ...
  for (iv = 0; iv < NSIMDVL; iv++) {
    fe_surf_oft_str(fe, index + iv, s1);
    for (ia = 0; ia < 3; ia++) {
      for (ib = 0; ib < 3; ib++) {
	s[ia][ib][iv] = s1[ia][ib];
      }

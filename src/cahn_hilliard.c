...
__host__ int ch_flux_mu_ext(ch_t * ch, field_t * vesicle_map, int outside_only);
...
__global__ void ch_flux_mu_ext_kernel(kernel_ctxt_t * ktx, ch_t * ch,
				   ch_info_t info, field_t * vesicle_map, int outside_only);
...
__host__ int ch_solver(ch_t * ch, fe_t * fe, field_t * phi, hydro_t * hydro,
  ...
  ch_flux_mu_ext(ch, vesicle_map, outside_only);
...
__host__ int ch_flux_mu_ext(ch_t * ch, field_t * vesicle_map, int outside_only) {
  ...
  tdpLaunchKernel(ch_flux_mu_ext_kernel, nblk, ntpb, 0, 0,
                  ctxt->target, ch->target, *ch->info, vesicle_map->target, outside_only);
  tdpAssert(tdpPeekAtLastError());
  tdpAssert(tdpDeviceSynchronize());

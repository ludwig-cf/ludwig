...
/*****************************************************************************
 *
 *  bond_harmonic2_create
 *
 *****************************************************************************/

int bond_harmonic2_create(pe_t * pe, cs_t * cs, bond_harmonic2_t ** pobj) {

  bond_harmonic2_t * obj = NULL;

  assert(pe);
  assert(cs);
  assert(pobj);

  obj = (bond_harmonic2_t *) calloc(1, sizeof(bond_harmonic2_t));
  assert(obj);
  if (obj == NULL) pe_fatal(pe, "calloc(bond_harmonic2_t) failed\n");
...
int bond_harmonic2_register(bond_harmonic2_t * obj, interact_t * parent) {
  ...
  interact_potential_add(parent, INTERACT_BOND_HARMONIC2, obj, bond_harmonic2_compute);
  interact_statistic_add(parent, INTERACT_BOND_HARMONIC2, obj, bond_harmonic2_stats);

/* -----> CHEMOVESICLE V2 */
/* Vesicle size constraint on cell width is now done by colloids_rt.c */
  interact_rc_set(parent, INTERACT_BOND_HARMONIC2, obj->r0);
/* <----- */
...
/*****************************************************************************
 *
 *  bond_harmonic2_compute
 *
 *****************************************************************************/

int bond_harmonic2_compute(colloids_info_t * cinfo, void * self) {

  bond_harmonic2_t * obj = (bond_harmonic2_t *) self;
  ...
  for (; pc; pc = pc->nextlocal) {
    ...
    if (pc->s.nbonds2 == 0) continue;

    for (n = 0; n < pc->s.nbonds2; n++) {
      //pe_verbose(cinfo->pe, "index %d, nbonds2 %d bonded %d %d %d %d %d %d \n", pc->s.index, pc->s.nbonds2, pc->bonded2[0]->s.index, pc->bonded2[1]->s.index, pc->bonded2[2]->s.index, pc->bonded2[3]->s.index, pc->bonded2[4]->s.index, pc->bonded2[5]->s.index);
      assert(pc->bonded2[n]);
      if (pc->s.index > pc->bonded2[n]->s.index) continue;

      /* Compute force arising on each particle from single bond */

      cs_minimum_distance(obj->cs, pc->s.r, pc->bonded2[n]->s.r, r12);
      r2 = r12[X]*r12[X] + r12[Y]*r12[Y] + r12[Z]*r12[Z];
      r2md = sqrt(r2);

      if (r2 < r2min) r2min = r2;
      if (r2 > r2max) r2max = r2;
      if (r2 > 4*obj->r0*obj->r0) pe_fatal(obj->pe, "Broken harmonic2 bond\n");
      ...
      pc->bonded2[n]->force[X] += f*r12[X];
      pc->bonded2[n]->force[Y] += f*r12[Y];
      pc->bonded2[n]->force[Z] += f*r12[Z];
    }
  }

...
__host__ int cahn_stats_reduce(phi_ch_t * pch, field_t * phi, map_t * map,
                               phi_stats_t * stats, int root, MPI_Comm comm) {
  ...
  tdpAssert(
      tdpMemcpy(stats_d, stats, sizeof(phi_stats_t), tdpMemcpyHostToDevice));

  {
    ...
    if (pch->info.conserve) {
      tdpLaunchKernel(cahn_stats_klein_sum_kernel, nblk, ntpb, 0, 0, k3d,
                      phi->target, pch->csum->target, map->target, stats_d);
      tdpAssert(tdpPeekAtLastError());
    }
    else {
      tdpLaunchKernel(cahn_stats_kahan_sum_kernel, nblk, ntpb, 0, 0, k3d,
                      phi->target, map->target, stats_d);
      tdpAssert(tdpPeekAtLastError());
    }

    tdpLaunchKernel(cahn_stats_var_kernel, nblk, ntpb, 0, 0, k3d, phi->target,
                    map->target, stats_d);
    tdpAssert(tdpPeekAtLastError());

    tdpLaunchKernel(cahn_stats_min_kernel, nblk, ntpb, 0, 0, k3d, phi->target,
                    map->target, stats_d);
    tdpAssert(tdpPeekAtLastError());
  ...
  tdpAssert(
      tdpMemcpy(&local, stats_d, sizeof(phi_stats_t), tdpMemcpyDeviceToHost));

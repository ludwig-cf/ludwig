...
int field_phi_init_rt(pe_t * pe, rt_t * rt, field_phi_info_t param,
  ...
  if (p != 0 && strcmp(value, "spinodal_patches") == 0) {
    int seed = DEFAULT_SEED;
    int patch = DEFAULT_PATCH_SIZE;
    double volminus1 = DEFAULT_PATCH_VOL;
      double volminus2 = DEFAULT_PATCH_VOL2;
    ...
    rt_double_parameter(rt, "phi_init_patch_vol", &volminus1);
      rt_double_parameter(rt, "phi_init_patch_vol2", &volminus2);
    field_phi_init_spinodal_patches(phi, seed, patch, volminus1,volminus2);
  }
  ...
  if (p != 0 && strcmp(value, "from_file") == 0) {
    ...
    if (p != 0 && strcmp(value, "ternary_X") == 0) {
       ...
       // rt_double_parameter(rt, "phi_init_block_dimension", &block_dimension);
       // pe_info(pe, "Initialising phi as block of size %0.2f along the X axis\n", block_dimension);
        field_phi_init_ternary_X(phi);
    }
    if (p != 0 && strcmp(value, "ternary_XY") == 0) {
        // double block_dimension = 10.0;
        // rt_double_parameter(rt, "phi_init_block_dimension", &block_dimension);
        // pe_info(pe, "Initialising phi as block of size %0.2f along the X axis\n", block_dimension);
        field_phi_init_ternary_XY(phi);
    }
    if (p != 0 && strcmp(value, "ternary_bbb") == 0) {
        // double block_dimension = 10.0;
        // rt_double_parameter(rt, "phi_init_block_dimension", &block_dimension);
        // pe_info(pe, "Initialising phi as block of size %0.2f along the X axis\n", block_dimension);
        field_phi_init_ternary_bbb(phi);
    }
    if (p != 0 && strcmp(value, "ternary_ggg") == 0) {
        // double block_dimension = 10.0;
        // rt_double_parameter(rt, "phi_init_block_dimension", &block_dimension);
        // pe_info(pe, "Initialising phi as block of size %0.2f along the X axis\n", block_dimension);
        field_phi_init_ternary_ggg(phi);
    }

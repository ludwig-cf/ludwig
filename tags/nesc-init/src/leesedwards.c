...
static void   LE_init_distribs_tosteadystate( Float );
...
/*----------------------------------------------------------------------------*/
/*!
 * Initialise velocity for Lees-Edwards (see assumptions in Notes below)
 *
 *- \c Options:   _TRACE_
 *- \c Arguments: none
 *- \c Returns:   none
 *- \c Buffers:   no dependence
 *- \c Version:   2.0b1
 *- \c Last \c updated: 04/03/2002 by JCD
 *- \c Authors:   JC Desplat
 *- \c See \c also: LE_init(), MODEL_init(), MODEL_set_velocity()
 *- \c Note:      this routine is suitable for the serial, OpenMP, and MPI
 *                implementations. Note also that there are a number of 
 *                restrictions/assumptions associated with the use of this
 *                routine:
 *                1. obviously, you can only call this routine *AFTER* the 
 *                   Lees-Edwards has been initialised, i.e., after calling 
 *                   LE_init()
 *                2. this routine is not suitable for a general case! LE walls
 *                   are expected to have identical velocities, and planes must
 *                   be placed the same distance apart
 *                This routine may be generalised later if required (low 
 *                priority) 
 */
/*----------------------------------------------------------------------------*/

/*JCD: needs validating and cleaning up!!! */

void LE_init_distribs_tosteadystate( Float transvel )
{
  int     ind, xfac, yfac, i, j, k;
  int     LE_loc, LE_loc2, LE_dist, dist, min_dist, plane, base_plane;
  Float   LE_vel, ugrad;
  int     N[3];
  int     offset[3];
  FVector u;
  ...
  LUDWIG_ENTER("LE_init_distribs_tosteadystate()");
  ...
  /* Only relevant if Lees-Edwards walls have been selected */
  if (N_LE_total == 0) {
    return;
  }
  
  /* Do not execute if we are re-starting a simulation (typically from disc) */
  if (get_step() != 0) {
    return;
  }
  ...
  /* 
   * Initialise velocities for Lees Edwards (see restrictions below):
   * 1. obviously, you can only call this routine *AFTER* the Lees-Edwards has
   *    been initialised, i.e., after calling LE_init()
   * 2. this routine is not suitable for a general case! LE walls are expected
   *    to have identical velocities, and planes must be placed the same 
   * distance apart
   *
   * This routine may be generalised later if required (low priority) 
   */
  
  /* Check that all LE velocities are identical */
  for(i=0; i < N_LE_total; i++)
    if (LeesEdw[i].vel != LeesEdw[0].vel) {
      verbose("LE_init_distribs_tosteadystate(): different LE_vel\n");
      return;
    }
  
  LE_vel = LeesEdw[0].vel; /* All LE velocities are identical! */

  /* Calculate velocity gradient (same for all LE walls) */
  /* Simple case: only 1 LE wall */
  if (N_LE_total == 1){
    ugrad = LE_vel / N_total(X);
  }
  /* More general case: > 1 LE wall*/
  else if(N_LE_total > 1)
    {
      /* Check identical spacing between Lees-Edwards walls */
      LE_dist = (LeesEdw[0].loc+N_total(X))-LeesEdw[N_LE_total-1].loc;

      for(i=0; i< N_LE_total-1; i++)
	if ((LeesEdw[i+1].loc-LeesEdw[i].loc) != LE_dist) {
	  verbose("LE_init_distribs_tosteadystate(): unequal wall spacing\n");
	  return;
	}

      ugrad = LE_vel / LE_dist;
    }
    
  /* Set grad_u[][] */
  grad_u.x.x =              grad_u.x.z = 0.0;
  grad_u.y.x = grad_u.y.y = grad_u.y.z = 0.0;
  grad_u.z.x = grad_u.z.y = grad_u.z.z = 0.0;
  ...
  /*
   * If reaching this point, then everything is okay: simply confirm the slope
   * for the ramp and proceed with the initialisation 
   */
  if(pe_rank() == 0){
  ...
  if(N_LE_plane == 0)
    {
      /* Look for closest LE plane (on neighbouring PE) to local origin */
      min_dist = N_total(X)+3;
      for(plane=0; plane < N_LE_total; plane++)
	{
	  dist = abs(LeesEdw[plane].loc-offset[X]);
	  if(dist<min_dist)
	    {
	      min_dist = dist;
	      base_plane = plane;
      ...
      /* Check on which side this plane lies */
      LE_loc = LeesEdw[base_plane].loc-offset[X];
      for(i=0; i<=N[X]+1; i++)
	{
	  /* Plane on RHS */
	  if(LE_loc > 0){ 
	    u.y = ugrad * (i-LE_loc-0.5) + 0.5*LE_vel;
	  }
	  /* Plane on LHS */
	  else{ 
	    u.y = ugrad * (i-LE_loc-0.5) - 0.5*LE_vel;
	  ...
	  for(j=0; j<=N[Y]+1; j++)
	    for(k=0; k<=N[Z]+1; k++)
	      {
		ind = i*xfac + j*yfac + k;
		MODEL_set_distributions_atrest(u, grad_u, site+ind);
	      }
	}
    }  
  
  /* At least 1 LE plane intersects the local domain */
  else
    {
      /* Deal with 1st subdomain domain: start of box -> 1st local LE wall */
      LE_loc = LeesEdw_plane[0].loc;
      for(i=0; i<=LE_loc; i++)
	{
	  u.y = ugrad * (i-LE_loc-0.5) + 0.5*LE_vel;
	  for(j=0; j<=N[Y]+1; j++)
      ...
      /* Then, deal with central sub-domains (between 1st and last LE wall) */
      for(plane=0; plane<N_LE_plane-1; plane++)
	{
	  LE_loc  = LeesEdw_plane[plane].loc;
	  LE_loc2 = LeesEdw_plane[plane+1].loc;
	  for(i=LE_loc+1; i<=LE_loc2 ; i++)
	    {
	      u.y = ugrad * (i-LE_loc-0.5) - 0.5*LE_vel;
	      for(j=0; j<=N[Y]+1; j++)
		for(k=0; k<=N[Z]+1; k++)
      ...
      /* Finally, deals with the final sub-domains (last LE wall -> end) */
      LE_loc  = LeesEdw_plane[N_LE_plane-1].loc;
      for(i=LE_loc+1; i<=N[X]+1 ; i++)
	{
	  u.y = ugrad * (i-LE_loc-0.5) - 0.5*LE_vel;
	  for(j=0; j<=N[Y]+1; j++)
	    for(k=0; k<=N[Z]+1; k++)

...
\label{spectrum}
\end{equation}
whereas we subtracted the average stress per cycle $\bar{\Pi}_{xy}$ from the signal.
...
The top picture shows the date over time, whereas the graph at the bottom gives 
...
right-handed the pattern becomes a mirror image of the former one. The sign of 
...
corresponds to Ericksen numbers of ${\cal E}\simeq10$, based on our
...
they become  irregularunregular and finally disappear after a few
...
positive as the background viscosity has been subtracted) and positive
...
Adjacent to regime BPI-1 ($\gd<3.91\e{-5}$) at 
at slightly larger shear rates $3.91\e{-5}\le\gd< 6.25\e{-4}$
and Ericksen numbers $0.5\le{\cal E}<10$ 
lies another region where the network flows with periodically 
recurring flow-induced conformations (viz. Fig. \ref{bp1-rheo}). 
We refer to this region as BPI-2. The transition between BPI-1 and BPI-2 
is shown in Fig. \ref{bp1-1_bp1-2} by means of the deviatoric stress over total strain. 
Obviously a qualitative difference between these two is the absence of 
...
temporarily negative (viz. Fig. \ref{bp1-fe-yield}), which 
...
kinetic path to bypass these large amplitude fluctuations and 
...
less straightforward to determine. In flow direction a multiple of $1/\gd$,
i.e. $\tau_F=n/\gd, n\in\cal{N}$ generally holds, e.g. $n=1,2,4$ for $\gd=3.91\e{-5},
7.81\e{-5},1.563\e{-4}$. The analysis is complicated by the fact
...
However, there is no such thing as a definite recurrence period $\tau_V$ 
in vorticity direction. The direction and magnitude of the displacement
in other directions (i.e. both in gradient and vorticity direction) 
is random, non-integer and depends sensitively on the imposed shear rate,
suggesting BPI-2 constitutes an instance of rheochaos.
...
Fig. \ref{bp2-velo}, but differs in the aspect that there is no
...
The intricacy and periodically recurrence of the flow-induced conformations 
...
Unfortunately the computational costs of simulations, given that large strains have to
...
It takes BPI also considerably longer in the smaller run to reach the 
...
Fig. \ref{bp1-high}. The snapshots visualise nicely the affine 
transformation of simple shear flow in gradient-flow plane.
...
Ericksen numbers ${\cal E}\simeq 10$.
...
of cubic BPs in simple shear flow. Although experimental evidence of our results 
is currently not available, we believe this work can shed some light into the 
...
At slightly larger Ericksen numbers $0.1<{\cal E}<10$ shear thinning is 

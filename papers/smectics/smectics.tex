...
\footnotetext{\textit{$^{a}$ Centre for Computational Science, University College London, INSERT ADDRESS HERE, UK}}
...
SADLY, I THINK IT WOULD BE BEST TO REWRITE THIS PAPER WITH THE FLOW DIRECTION ALONG X, THE GRADIENT ALONG Y AND THE VORTICITY ALONG Z AS IS COMMONLY USED IN THE RHEOLOGICAL LITERATURE! I HAVE NOT STARTED TO MAKE THIS CHANGE AS IT WOULD CONFUSE THINGS TOO MUCH AND THE JOB REQUIRES CARE. ALSO THE CHEMICAL PRESSURE AND THE STRESS HAVE OPPOSITE SIGNS YET THE FIGURES SHOW THEM HAVING THE SAME SIGN. WHAT HAS HAPPENED?
...
Finally, in common with other complex fluids, lamellar phases can undergo transitions into heterogeneous states, for instance to form shear bands (layers of unequal $\sigma_{xy}$ or $\dot\gamma$) [P D Olmsted, Rheologica Acta 47, 283-300 (2008)]. 
...
marked shear thinning as this structure is sheared out at high flow rates [Larson book -- or Davide to supply references]. 
...
and hence to a large viscosity in any geometry where flow necessitates such motion [deGennes book]. (A well-known example of permeation flow
is found in cholesterics which are sheared or pushed along the direction
of the cholesteric helix.) Permeation is often involved in defect motion such as the growth or shrinkage of incomplete layers at the sites of edge dislocations, and may also be implicated in other forms of topological reconstruction. Such reconstruction arises under shear, sometimes to an extreme degree, for instance in the shear-induced transition from an ordered lamellar stack to an onion phase in lyotropic smectics \cite{Diat93,Panizza96,Iwashita07}[ADD; M Gradzielski, JPCM 15, R655-R697 (2003)]. (The onion phase comprises a space-filling packing of multilayer vesicles; under some conditions similar defect textures can arise without shear \cite{Gomati87,Boltenhagen92,Fournier94,Ramos04}.) The mechanism underlying such global reconstructions under shear of the layer organization remains unclear. Most theoretical predictions have focussed on buckling and other instabilities of perfectly ordered smectics as the initial trigger for onion formation [A G Zilman and R Granek, EPJB 11, 593-608 (1999), L Courbin, J P Delville, J Rouch and P Panizza, PRL 89, 148305 (2002)]\cite{Guo2002,Soddemann2004, Stewart2009} but the pathway may also strongly involve the physics of defect motion [J Zipfel, J Berghause, P Lindner, W Richtering, J Phys Chem B 103, 2841-2849 (1999), A Leon, D Bonn, J Meunier, A Al-Kahlwaji, O Greffer, H Kellay, PRL 84, 1355-1338 (2000)]. Although we do not see onions in the simulations reported here, in two dimensions we do see a transition from a well ordered phase to a defect-rich phase, whose possible link to onion formation deserves to be explored in future work. Another interesting avenue connects with the observation, in block copolymers and some lyotropic surfactant systems, of a shear-induced
...
[CITE OUR PREVIOUS PAPERS USING HYBRID LB, ALSO THAT OF GONELLA ET AL WHO FIRST USED IT FOR SMECTICS].
...
The equilibrium phase in a system of volume $V$ is found for an order parameter field $\phi({\bm r})\equiv \phi$ that minimises the Landau-Brazovskii free energy functional \cite{Brazovskii75} [ADD: Gommper and Schick].
%
\begin{eqnarray}
...
%
\begin{eqnarray}
\Delta p&=& \frac{a}{2}\phi^2+\frac{3b}{4}\phi^4-\kappa\left(\phi(\nabla^2\phi)+\frac{1}{2}({\nabla}\phi)^2\right)\nonumber\\
...
\begin{equation}
Pe^* =\frac{(a_c+b)a_c}{4b^2}\,\frac{L^2}{\eta M}\label{pestar}
\end{equation}
With our chosen parameters (in LBU, $a_c=1.2\times 10^{-4}, L = 10, \eta = 8.3\times 10^{-2}, M = 0.25$) we find $Pe^* \sim 1400$ at $\beta = 2.1$ and $Pe^* \sim 40000$ at $\beta = 0.21$. The maximum values of $Pe/Pe^*$ that we address below are of order 0.1 and 0.04 respectively in the two cases.
...
Below we first present detailed results for two parameter sets ($\beta = 2.1, 0.21$) in two dimensions which we call systems A-2D and B-2D. System lattice sizes and shear rates are reported in Table \ref{tab1}; all remaining parameters are the same in both systems as detailed above. 
...
A third parameter set ($\beta = 0.42$, system C-2D) gave results broadly similar to B-2D. Note that we will later report simulations in three dimensions for systems A-3D and C-3D which share parameters with A-2D and C-2D (see Table \ref{tab1}).

\begin{table}[htp]
\small
\caption{Simulation parameters: After initialization from a noisy uniform state, the systems were first equilibrated at zero shear rate (EQ), followed by a sequence of various shear rates, ranging from $\dot{\gamma}=1.25\times10^{-5}$ to $2\times10^{-4}$LBU. In some cases we performed a final switch off run (SO) at zero shear rate. The parameters $\kappa=-6\times10^{-4}, c=7.6\times10^{-4}, M=0.25$ and $\eta=8.33\times10^{-2}$ were the same throughout all simulations. ADD PE and CA COLUMNS!}
\label{tab1}
\begin{tabular*}{0.495\textwidth}{lllll}
\hline
run \# & $N_x\times N_y\times N_z$ & time $/10^3$ & $10^5b$ &  $10^5\dot\gamma$\\
\hline
A & & & &\\
\hline 
703 & $512\times1024\times1$ & 0-320 & $50$ & 0 - EQ  \\
704 & $512\times1024\times1$ & 320-640& $50$ & $10$\\
705 & $512\times1024\times1$ & 640-960& $50$ & 0 - SO \\
\hline
767 & $256\times512\times1$ & 0-320 & $50$  & 0 - EQ\\
775 & $256\times512\times1$ & 320-640& $50$ & $20$ \\
769 & $256\times512\times1$ & 320-2560& $50$ & $10$ \\
771 & $256\times512\times1$ & 1280-3840& $50$ & $ 5$ \\
772 & $256\times512\times1$ & 5120-7680& $50$ & $ 2.5$ \\
774 & $256\times512\times1$ & 5120-10240& $50$ & $1.25$ \\
\hline
792 & $128\times256\times1$ & 320-1600& $50$ & $10$ \\
\hline
786 & $128\times256\times128$ & 0-640 & $50$ & 0 - EQ\\
787 & $128\times256\times128$ & 320-640 & $50$ & $20$\\
788 & $128\times256\times128$ & 320-960 & $50$ & $10$\\
789 & $128\times256\times128$ & 640-1280 & $50$ & $5$\\
\hline
B & & & &\\
\hline
707 & $512\times1024\times1$ & 0-320 & $5$ & 0 - EQ \\
710 & $512\times1024\times1$ & 320-960& $5$ & $10$ \\
765 & $512\times1024\times1$ & 960-1280& $5$ & 0 - SO \\
\hline
776 & $256\times512\times1$ & 0-320 & $5$ & 0 - EQ\\
777 & $256\times512\times1$ & 320-960& $5$ & $20$ \\
778 & $256\times512\times1$ & 320-2560& $5$ & $10$ \\
780 & $256\times512\times1$ & 1280-3840& $5$ & $5$ \\
781 & $256\times512\times1$ & 2560-6400& $5$ & $2.5$ \\
782 & $256\times512\times1$ & 5120-7680& $5$ & $1.25$ \\
\hline
C & & & &\\
\hline
998 & $128\times256\times1$ & 0-320& $10$ & 0 - EQ \\
1001 & $128\times256\times1$ & 320-960& $10$ & $20$ \\
1000 & $128\times256\times1$ & 320-1280& $10$ & $10$ \\
999 & $128\times256\times128$ & 0-320 & $10$ & 0 - EQ\\
1003 & $128\times256\times128$ & 320-960 & $10$ & $20$\\
1002 & $128\times256\times128$ & 320-1280 & $10$ & $10$\\
\hline
\end{tabular*}
\end{table}
...
We turn now to system A-2D ($\beta = 2.1$) The top panel in Fig. \ref{fig1} shows the order parameter pattern $\phi$ at the end of the equilibration phase prior to shearing.
...
Fig. \ref{fig2} shows equivalent states for system B-2D ($\beta = 0.21$).
The Peclet number during the shearing phase is now $Pe = 800$ $(Ca = 0.02)$ CONFIRM.
...
\caption{Defect structure of system A-2D:  at timestep $t=5.12\e{6}$ (top) and $1.024\e{7}$ (bottom) with an imposed shear rate $\dot{\gamma}=1.25\times10^{-5}$. The system has been pre-sheared at stepwise decreasing rates (see Table \ref{tab1}). Sites where the alignment function $X$ is smaller than $0.8$ are highlighted red.}
\label{fig3}
\end{figure}
...
The upper panel gives the situation at the lowest applied shear rate $\dot{\gamma}=1.25\e{-5}$, immediately following a pre-shearing protocol in which the strain rate was ramped down stepwise from higher values (see Table \ref{tab1}) IS THIS CROPPED OR THE WHOLE SYSTEM? WHAT SIZE?. While large aligned regions are present there are other regions of high defect density.
...
Defect annihilation to this large extent is only found at low enough shear rates. A prerequisite seems to be that the defects can smoothly approach each other and then annihilate. This is prevented by the buckling of layers above a certain shear rate, beyond which the system shows a tendency to trip over itself, which results in the creation of new defects. The onset of defect proliferation arises at characteristic flow rates $\dot{\gamma}_c=2.5-5\e{-5}$LBU in system A-2D and around $\dot{\gamma}_c=1-2\e{-4}$LBU in B-2D. 
...
The pictures show the order parameter $\phi({\bm r})$, shear stress density $\sigma_{xy}({\bm r})$ and first normal stress difference density $N_1({\bm r})=\sigma_{yy}({\bm r})-\sigma_{xx}({\bm r})$ of system A-2D in steady shear flow at shear rate $\dot{\gamma}=1\e{-4}$ ($Pe = XX$, $Ca = YY$). (Here and below we adopt a COMPLETELY UNCONVENTIONAL coordinate system where $y$ is along the flow direction and $x$ the velocity gradient.)
...
\caption{Structure factor $C({\bm k})$ of system A-2D: Shown are states after equilibration at $t=3.2\e{5}$ (top); in steady shear at $\dot{\gamma}=1\times10^{-4}$ and $t=5\e{5}$ (centre) and after switch-off at $t=1.28\e{6}$ (bottom). The sections shown correspond to wave vectors on the interval $k_x, k_y \in [-\pi/2,\pi/2 ]$ LBU. LABEL THE AXES OTHERWISE X AND Y DIRECTIONS ARE NOT SPECIFIED.}
\label{fig5}
\end{figure}
...
Interestingly, following each step down in shear rate, both systems A-2D and B-2D continuously anneal to lower defect densities on a timescale $\simeq 10^6$LBU CONFIRM FOR B. If the shear rate is turned up again (not shown), the lamellae buckle and new defects recur on a timescale $XXX$LBU. 
...
\caption{Average total shear stress $\sigma_{xy}$ versus time for system A-2D: The horizontal lines are steady state time averages. For shear rate $\dot{\gamma}=1.25\e{-5}, 2.5\e{-5}, 5\e{-5}, 1\e{-4}, 2\e{-4}$, we find $\eta_{app}/\eta\equiv\sigma_{xy}/ \eta \dot{\gamma}=2.49, 2.25, 2.18, 1.93, 1.74$.
 }
\label{fig7}
\end{figure}
...
\caption{Average order parameter contribution to the pressure tensor $P_{xy}$ vs time for system A-2D. The inset shows data for system A-3D and shear rates $\dot{\gamma}=2\times10^{-4}, 1\times10^{-4}$ and $5\times10^{-5}$ (left to right).}
\label{fig8}
\end{figure}
...
\caption{Average chemical contribution to the pressure tensor $ P_{xy}$ vs. defect density for system A-2D. REMOVE B-2D. PROBABLY MERGE 7, 8, 9 INTO SINGLE FIGURE.} 
\label{fig9}
\end{figure}

System A-2D shows clear shear thinning behaviour; values of its apparent viscosity at various shear rates are given in the caption of Fig.~\ref{fig7}. The corresponding range of $Ca$ is XXXX-YYYY. Using Eq.\ref{fullstress}, we can identify the stress contribution arising from the deviatoric part of the order parameter pressure tensor
$\langle P_{xy}({\bf r})\rangle\equiv P_{xy}$; this is shown in Fig.~\ref{fig8} RESOLVE SIGN ISSUE. The temporal fluctuations are in phase with those of the total stress in Fig.~\ref{fig7} and decrease with decreasing shear rate.
Fig. \ref{fig9} plots $P_{xy}$ against the defect density $\rho_D$; this shows a linear relation across almost the whole range of shear rates.

In contrast, system B-2D does not exhibit clear shear thinning; the apparent viscosities obey $\eta_{app}/\eta=1.57, 1.62, 1.55, 1.42, 1.87$ for the same set of shear rates as used in Fig.~\ref{fig7}. To within the measured accuracy, system B-2D is rheologically quasi-Newtonian. However, the range of $Ca$ in this case is lower, ZZZZ-TTTT, and the order parameter stress creates only a relatively modest uplift to the baseline viscosity. More significantly, $Ca\le Ca_c$ throughout this range CONFIRM so that the steady states found in system B-2D are far more ordered than in A-2D. This results from a defect annihilation process, occurring in the early stages of ramp-down, which we found to be accompanied by a sharp drop in $P_{xy}$ (not shown).

\begin{figure}[htp!]
\centering
\includegraphics[angle=0,width=0.5\textwidth]{N1_t_5e-4.pdf}
\caption{Spatially averaged first normal stress differences $N_1= \sigma_{yy}-\sigma_{xx}$ vs time for system A-2D. The inset shows data for system A-3D and shear rates $\dot{\gamma}=2\times10^{-4}, 1\times10^{-4}$ and $5\times10^{-5}$ (left to right). Horizontal solid lines are time averages.}
\label{fig10}
\end{figure}

Fig.~\ref{fig10} shows averaged first normal stress differences $\langle N_1({\bf r})\rangle \equiv N_1$ versus time for system A-2D, where we followed the same stepwise decreasing flow protocol as before.
...
THAT MAKES THE RESULT FOR A-2D STILL MORE SURPRISING, NO?

In neither system does $\sigma_{xy}(\dot\gamma)$ offer any clear signature of a yield stress, as would be expected to arise for a frozen-in defect texture in the static state. This can be understood if, as detailed previously, that texture is replaced by a fully aligned state at the lowest shear rates studied here: with the layering wavevector perpendicular to the flow, such a state should have a purely viscous response. On the other hand, one would expect $N_1$ also to vanish in that case; if this happens, it is at lower strain rates than encountered here. WE HAVE TO UNDERSTAND WHY THE NORMAL STRESS IS SO LARGE, ESPECIALLY IN B-2D WHERE YOU ARE CLAIMING THAT THE CHEMICAL STRESS IS NEGLIGIBLE!!

\subsection{Three Dimensions}
\label{results3D}

The computational cost of repeating all the above studies in three dimensional systems of equivalent size remains prohibitive, particularly when addressing steady state properties which emerge only after a million or more timesteps. We therefore focus in 3D on smaller systems (Table \ref{tab1}), but still large enough in principle to show qualitatively similar structure under flow. Our studies use two sets of parameters. System A-3D is identicaly to A-2D except for the introduction of the third dimension. System C-3D has parameters intermediate between A-2D and B-2D (Table \ref{tab1}) and was chosen in preference to the latter because 
preliminary studies on B-3D showed very rapid alignment into a fully ordered state. CONFIRM THAT, OR GIVE ANOTHER GOOD REASON. (We see below however that in fact this is a rather general outcome in 3D.)
...
\caption{Structure factor $C({\mathbf k})$ of quiescent system A-3D, showing a cut along $k_y=0$ after equilibration at $t=3.2\e{5}$. Shown are wave vectors on the interval $k_x, k_z\in[-\pi/2 ,\pi/2 ]$. Data for C-3D (not shown) is almost the same. CONFIRM}
\label{fig13}
\end{figure}
...
\caption{Structure factor $C({\mathbf k})$ of system A-3D in shear flow ($\dot{\gamma}=1\times10^{-4}$) at t=960k: Cuts through along $k_x=0,k_y=0$ and $k_z=3\pi/16 $, respectively are shown for wave-vectors $k_x, k_y, k_z\in[-\pi/2 ,\pi/2]$. Note that in these images y is the flow direction, whereas the direction of the velocity gradient and vorticity are z and x, respectively.}
\label{fig14}
\end{figure}

Fig.~\ref{fig12} shows the post-equilibration amorphous structure and the steady-state structure under flow in system C-3D (A-3D is similar) at system size $N_x\times N_y \times N_z=128\times256\times128$. The latter is compared with C-2D and A-2D in a system of the same $x$ and $y$ dimensions ($128\times256\times1$).  The quiescent metastable state of system C-3D after equilibration is more or less the same as it would be in two dimensions. 
Indeed, a slice through the structure factor along $k_y=0$ (cuts along $k_x,k_z=0$ look identical) is shown in Fig. \ref{fig13} and resembles that in Fig.
~\ref{fig5}.
However, under shear at $\dot\gamma = 2\times 10^{-4}$ ($Pe = XXX$, $Ca = YYY$) this structure anneals readily into a regular lamellar structure after a transient period of about XXX timesteps. This occurs for even the highest shear rates applied, indicating that there is a very strong tendency in three dimensions to form ordered lamellar arrays. The same behaviour was seen CONFIRM in system A-3D for which the highest flow rate used corresponds to $Pe = XXX$ and $Ca = YYY$. (In contrast, the 2D results at the same flow rates and system sizes feature the typical fluctuating pattern of breaking and merging lamellae that was described previously and is visible in the lower panels of Fig.~\ref{fig12}.) The structure factor seen in 3D is accordingly close to that of a perfectly ordered lamellar state (see Fig.~\ref{fig14}) in contrast to the corresponding results in 2D (Fig.~\ref{fig5}).
These findings suggest that any characteristic strain rate $\dot\gamma_c$ at which defects proliferate must be markedly higher in 3D than in 2D. Specifically, if $Ca$ is the relevant control parameter, our results for A-3D imply that $Ca_c > YYY$ in three dimensions.

Note however that the order we observe is not completely perfect and edge dislocations emerge spontaneously in pairs as a consequence of a mismatch between some of the layers. WHAT DOES THAT MEAN?
...
This is a direct consequence of the additional dimension and seems to play a key role in promoting the formation of the homogeneously layered structure.
I DO NOT UNDERSTAND ALL THIS: DOES IT CORRESPOND TO MOTION OF THE ORIENTED EDGE DISLOCATIONS? IF NOT, WHAT ROLE DO THOSE PLAY DURING THE ANNEALING?
EXPLAIN CAREFULLY WHY THIS MECHANISM SPEEDS THINGS UP IN 3D BUT NOT IN 2D.

We next quantify the time system A-3D takes to anneal into its final ordered structure starting from an isotropic initial configuration. (These and subsequent measurements are reported for A-3D only; C-3D is qualitatively similar CONFIRM.)  
...
The total strain where the curves level off, and ordering is achieved, scales with the shear rate and is about $\gamma=1.3$ (at $\dot{\gamma}=2\times10^{-4}$), $\gamma=0.8$ (at $\dot{\gamma}=1\times10^{-4}$) and $\gamma=0.6$ (at $\dot{\gamma}=5\times10^{-5}$). CONFIRM THESE STRAIN NUMBERS, THEY SEEM UNBELIEVABLY SMALL!!!

The reduced number of defects is reflected in the magnitude of the order parameter stress contribution $P_{xy}$; the 3D behaviour is shown in the inset of Fig.~\ref{fig8}. The final contribution to the total stress is created by a small number of remaining defects (hence the inconsistent trend of the tiny plateau value with strain rate, which is not a concern here CONFIRM) and is quite negligible compared to the background viscosity contribution $\eta\dot\gamma$. The drop in $P_{xy}$ by three orders of magnitude during the defect annihilation process can formally be viewed as shear thinning of a quite extreme degree -- albeit one masked, in any macroscopic measurement of $\sigma_{xy}$, by the Newtonian background. 

First normal stress differences $N_1=\langle\sigma_{yy}-\sigma{zz}\rangle$ for system A-3D are shown in the inset of Fig.\ref{fig10}. THIS IS THE WRONG DEFINITION EVEN WITH THE CRAZY COORDINATE SYSTEM ADOPTED IN THIS PAPER. N1 IS FLOW MINUS GRADIENT NOT FLOW MINUS VORTICITY.
...
CAN WE PLEASE MEASURE THE NORMAL STRESS IN A COMPLETELY DEFECT FREE STATE AND COMPARE WITH THE OBSERVED VALUES. DOES THE PLATEAU CORRESPOND TO A FEW REMAINING DEFECTS OR DOES IT STEM FROM THE PERFECT BEHAVIOUR??
...
Many earlier studies have proposed a picture in which a layer undulation instability in three-dimensional smectic liquid crystals emerges above a critical shear rate [A G Zilman and R Granek, EPJB 11, 593-608 (1999), L Courbin, J P Delville, J Rouch and P Panizza, PRL 89, 148305 (2002)]\cite{Gonnella98,Kumaran2001} [Ramaswamy]. Molecular dynamics simulations on thermotropic smectics place this in a regime of flow rates well above those we have applied in the present study \cite{Guo2002,Soddemann2004, Stewart2009}  CONFIRM THIS WITH NEW PARAMETER MAPPING -- IT COULD BE WRONG. The status of this instability and its effects on rheology in experiments remains somewhat unclear. Also unclear is its status in two dimensions.
...
\footnotesize{
RSC STYLE FILE APPARENTLY STRIPS OUT MIDDLE INITIALS. THIS IS OFFENSIVE, PARTICULARLY TO AUTHORS LIKE AJ WAGNER WITH COMMON NAMES.

...
static const int  cds_with_both  = 0;  /* Output both */
static const int  cds_with_both_and_forces  = 1;  /* Output both */
...
void colloids_to_csv_header_with_both_and_forces(FILE * fp);
...
int main(int argc, char ** argv) {
  ...
  if (cds_with_both_and_forces) colloids_to_csv_header_with_both_and_forces(fp_csv);
  ...
  for (nf = 1; nf <= nfile; nf++) {
	...
	fprintf(fp_csv, format7end_, s1.m[0], s1.m[1], s1.m[2], s1.v[0], s1.v[1], s1.v[2], normv);
      ...
      if (cds_with_both_and_forces) {
	normv = sqrt(s1.v[0]*s1.v[0] + s1.v[1]*s1.v[1] + s1.v[2]*s1.v[2]);
	fprintf(fp_csv, format13end_, s1.m[0], s1.m[1], s1.m[2], s1.v[0], s1.v[1], s1.v[2], normv, s1.force_colloid_fe[0], s1.force_colloid_fe[1], s1.force_colloid_fe[2], s1.force_colloid_bbl[0], s1.force_colloid_bbl[1], s1.force_colloid_bbl[2]);
      }
      ncount += 1;
...
/*****************************************************************************
 *
 *  colloids_to_csv_header_with_both_and_forces
 *
 *****************************************************************************/
...
void colloids_to_csv_header_with_both_and_forces(FILE * fp) {
  ...
  if (id) fprintf(fp, "%s", "id, ");
  fprintf(fp, "%s", "x, y, z, mx, my, mz, vx, vy, vz, normv, fx_fe, fy_fe, fz_fe, fx_bbl, fy_bbl, fz_bbl\n");

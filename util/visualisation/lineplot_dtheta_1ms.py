...
for i, (dtheta, label) in enumerate(zip([dthetas[0],dthetas[4], dthetas[8]], [labels[0], labels[4], labels[8]])):

  plt.plot(-dtheta, '--', label=label, color=cm.Blues((i+1)/4))
...
plt.tight_layout()
savename="dtheta_1m3.png"
plt.savefig(savename)
plt.clf()



for i, (dtheta, label) in enumerate(zip([dthetas[1],dthetas[5], dthetas[9]], [labels[1], labels[5], labels[9]])):
...
plt.legend(fontsize = 12)
plt.tight_layout()
savename="dtheta_1m4.png"
plt.savefig(savename)
plt.clf()


for i, (dtheta, label) in enumerate(zip([dthetas[2],dthetas[6], dthetas[10]], [labels[2], labels[6], labels[10]])):
...
plt.legend(fontsize = 12)
plt.tight_layout()
savename="dtheta_1m5.png"
plt.savefig(savename)
plt.clf()

for i, (dtheta, label) in enumerate(zip([dthetas[3],dthetas[7], dthetas[11]], [labels[3], labels[7], labels[11]])):
